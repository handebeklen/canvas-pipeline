...
def find_closest_probes(chrom, start, end, lrr_data):
    ...
    return left_probe, right_probe, j - i
...
def process_probes(bed_file, lrr_file, output_file):
    ...
    with open(bed_file, "r") as bed_f, open(output_file, "w") as out_f:
        for line in bed_f:
            ...
            left_probe, right_probe, snp_number = find_closest_probes(
                chrom, start, end, lrr_data
            )
...
def main():
    ...
    parser.add_argument(
        "--bed", required=True, help="Input BED file containing CNV regions"
    )

    parser.add_argument("--lrr", required=True, help="Input LRR data file")
