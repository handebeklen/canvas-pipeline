...
def process_bed_file(bed_file, bands):
    ...
    with open(bed_file, "r") as file:
        for line in file:
            ...
            chrom = (
                f"chr{columns[0]}" if not columns[0].startswith("chr") else columns[0]
            )
            start = int(columns[1])
            end = int(columns[2])
            cn = columns[3]  # Assuming CN is in the 4th column
            numsnp = columns[4]  # Assuming numsnp is in the 4th column
            ...
            results.append(f"{chrom}_{start}_{end}\tCN={cn}\tnumsnp={numsnp}\t{iscn}")
...
def main():
    ...
    input_group.add_argument("--cnv", "-c", help="Input CNV file")
    input_group.add_argument("--bed", "-d", help="Input BED file")
    parser.add_argument("--band", "-b", required=True, help="Cytogenetic band file")
